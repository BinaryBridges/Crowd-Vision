...
def draw_tracked_faces(frame: np.ndarray, tracked: List[TrackedFace]):
    for face in tracked:
        ...
        label = f"{face.label} {face.confidence:.2f} ({status_text})" if face.confidence > 0 else f"{face.label} ({status_text})"
        draw_text_label(frame, label, x1, y1)
    
def prune_old_notifications(notified: Dict[str, int], current_frame: int):
    expired = [name for name, frm in notified.items() if current_frame - frm > config.NOTIFY_COOLDOWN_FRAMES]
    for name in expired:
